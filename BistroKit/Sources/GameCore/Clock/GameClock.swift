...
/// Source of "now" for the whole game.
///
/// The engine never calls `Date()` itself. Tests and the balance simulator inject a
/// `ManualClock` so they can simulate hours of play in milliseconds.
...
public final class ManualClock: GameClock, @unchecked Sendable {
    ...
    /// Moves time forward. Negative values are ignored: game time never goes backwards.
