    ...
    steps:
      ...
      - name: Install certificate and provisioning profile
        ...
        run: |
          ...
          # App Store Connect API key, rebuilt as a clean PEM (secrets pasted with "\n" or CRLF still work).
          mkdir -p "$HOME/.appstoreconnect/private_keys"
          printf '%s' "$ASC_KEY_P8" > "$RUNNER_TEMP/asc_key_raw.txt"
          python3 scripts/ci/asc.py normalize-key "$RUNNER_TEMP/asc_key_raw.txt" \
            "$HOME/.appstoreconnect/private_keys/AuthKey_${ASC_KEY_ID}.p8"
          rm -f "$RUNNER_TEMP/asc_key_raw.txt"

          echo "profile_name=$PROFILE_NAME" >> "$GITHUB_OUTPUT"
          echo "team_id=$TEAM_ID" >> "$GITHUB_OUTPUT"
          echo "bundle_id=$BUNDLE_ID" >> "$GITHUB_OUTPUT"

      - name: Check the App Store Connect API key and app (before building)
        if: steps.secrets.outputs.can_sign == 'true'
        id: asc
        run: |
          export ASC_KEY_FILE="$HOME/.appstoreconnect/private_keys/AuthKey_${ASC_KEY_ID}.p8"
          # Fails (red) with an explicit reason if the key is refused (401) or if no app has this bundle ID.
          python3 scripts/ci/asc.py check-app "${{ steps.signing.outputs.bundle_id }}"
      ...
      - name: Check the IPA (path, bundle ID, version, build number, signature)
        if: steps.secrets.outputs.can_sign == 'true'
        id: ipa
        run: |
          set -euo pipefail
      ...
      - name: Confirm the build is in App Store Connect
        if: steps.secrets.outputs.can_sign == 'true'
        run: |
          export ASC_KEY_FILE="$HOME/.appstoreconnect/private_keys/AuthKey_${ASC_KEY_ID}.p8"
          # Waits (up to 20 min) until App Store Connect lists the build; fails if it never shows up
          # or if Apple marks it FAILED / INVALID.
          python3 scripts/ci/asc.py wait-build "${{ steps.asc.outputs.app_id }}" "${{ steps.version.outputs.build_number }}" 20
          {
            echo "### Build ${{ steps.version.outputs.build_number }} received by App Store Connect"
            echo "Bundle \`${{ steps.signing.outputs.bundle_id }}\` · version ${{ steps.ipa.outputs.version }}."
            echo "Apple is processing it (usually 5–30 min); it then appears in TestFlight."
          } >> "$GITHUB_STEP_SUMMARY"
